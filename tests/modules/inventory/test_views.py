...
from src.modules.inventory.models import (Category, Product, StockLevel,
                                          StockMovement, Warehouse)
from src.modules.inventory.views import (CategoryViewSet, ProductViewSet,
                                         StockLevelViewSet,
...
class TestWarehouseViewSet:
    ...
    def test_create_warehouse(self, db, api_client):
        data = {"name": "New Warehouse", "code": "NWH001", "address": "123 New St"}
...
class TestProductViewSet:
    ...
    def products(self, db, category):
        return [
            Product.objects.create(
                sku="PRD001", name="Product 1", category=category, unit_price=99.99
            ),
            Product.objects.create(
                sku="PRD002", name="Product 2", category=category, unit_price=49.99
            ),
        ]
    ...
    def test_stock_summary_action(self, db, api_client, products):
        ...
        response = api_client.get(
            f"/api/inventory/products/{product.id}/stock_summary/"
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestStockLevelViewSet:
    ...
    def setup_stock(self, db):
        ...
        stock = StockLevel.objects.create(
            product=product, warehouse=warehouse, quantity=100, reserved_quantity=20
        )
    ...
    def test_low_stock_action(self, db, api_client, setup_stock):
        ...
        StockLevel.objects.create(
            product=product, warehouse=warehouse, quantity=10  # Below reorder level
        )
...
class TestStockMovementViewSet:
    ...
    def setup_movement(self, db):
        ...
        category = Category.objects.create(name="Test Cat")
        product = Product.objects.create(
            sku="MOV001", name="Move Item", category=category, unit_price=15.00
        )
